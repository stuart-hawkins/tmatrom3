% Segment of an ellipsoid in R^3 parametrised from [0,1] x [0,1]
%
%  s = ellipsoid_segment(Itheta,Iphi,ax,ay,az) represents a segment of the
%  ellipsoid centred at the origin parametrised in spherical polar
%  coordinates
%
%    x = ax * sin(theta) * cos(phi)
%    y = ay * sin(theta) * sin(phi)
%    z = az * cos(theta)
%
%  The segment is that part of the ellipsoid with theta between Itheta(1) and
%  Itheta(2), and phi between Iphi(1) and Iphi(2). If Itheta = [0,pi] and
%  Iphi = [0,2*pi] then the segment is the whole ellipsoid.
%
...
%
% See also: sheet, translated_sheet, spherical_segment, disk.
%
% Stuart C. Hawkins - 25 August 2021
...
classdef ellipsoid_segment < sheet
    ...
    methods
        ...
        function self = ellipsoid_segment(thetalim,philim,ax,ay,az)
            ...
            % specify spherical polar coordinates parametrisation of the ellipsoid 
            % for the surface mapping....
            f = @(u,v) [ax*sin(theta(u)).*cos(phi(v));ay*sin(theta(u)).*sin(phi(v));az*cos(theta(u))];
            
            % ....and its derivative with respect to u (theta)...
            dtheta = @(u,v) [ax*thetad*cos(theta(u)).*cos(phi(v));ay*thetad*cos(theta(u)).*sin(phi(v));-az*thetad*sin(theta(u))];
            
            % ....and its derivative with respect to v (phi)...
            dphi = @(u,v) [-ax*phid*sin(theta(u)).*sin(phi(v));ay*phid*sin(theta(u)).*cos(phi(v));zeros(size(theta(u)))];
