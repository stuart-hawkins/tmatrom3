...
%
%  p = associatedLegendre(degree,x) instantiates an object to compute
%  associated Legendre functions.
%
%  p.setup(x) computes and stores the values of the associated Legendre
%  function at the points x.
%
%  P = p.get(n) for n<=degree returns the values P of the associated 
%  Legendre functions of degree n in a length(x) x (2*n+1) matrix.
%  P(:,m+n+1) holds the values of the polynomial of order m.
%
%  P = p.get(n,m) returns the values of the associated Legendre function of
%  degree n and order m at the points x.
%
% Note: the polynomials are normalised as in Equation (3.2) in
% Ganesh and Hawkins, Numerical Algorithms (2006) 43:25-60.
%
% See also: derivativeLegendre.
%
...
classdef associatedLegendre < handle
    ...
    methods
        ...
        function self = associatedLegendre(degree,x)
            
            self.degree = degree;
            self.data = [];
           
            % call setup method to compute the Legendre function values
            self.setup(x);
        ...
        % This computes the associated Legendre function values at the
        % points x and stores them for later use.
        ...
        function val = get(self,varargin)
            ...
            elseif nargin == 3
                ...
                error('self.get() method takes two or three parameters')
